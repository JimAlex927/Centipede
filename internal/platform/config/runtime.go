...
func validateHotReload(previous, next Config) error {
	if previous.Storage != next.Storage || previous.Mail != next.Mail || previous.Migration != next.Migration || previous.PDFOCR != next.PDFOCR {
		return errors.New("storage, mail, migration or PDF OCR settings changed; restart is required")
	}
