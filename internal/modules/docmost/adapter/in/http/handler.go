...
func (handler *Handler) Register(router gin.IRouter) {
	...
	api.GET("/sso/google/login", handler.googleLogin)
	api.GET("/sso/google/signup", handler.googleSignup)
	api.GET("/sso/google/callback", handler.googleCallback)
	...
	handler.registerSCIMProtocolRoutes(api)
