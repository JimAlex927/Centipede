...
describe("getFileUrl", () => {
  ...
  it("encodes attachment names when the backend does not return a URL", () => {
    expect(
      getAttachmentFileUrl({ id: "attachment-1", fileName: "流程 图.drawio.svg" }),
    ).toBe("http://localhost:7788/api/files/attachment-1/%E6%B5%81%E7%A8%8B%20%E5%9B%BE.drawio.svg");
  });
  ...
  it("requests protected cross-origin attachments with cookies", () => {
    expect(getFileCrossOrigin("/api/files/attachment-1/diagram.drawio.svg")).toBe(
      "use-credentials",
    );
    expect(getFileCrossOrigin("https://example.com/diagram.svg")).toBeUndefined();
  });

  it("recognizes legacy storage paths as internal attachments", () => {
    expect(isInternalFileUrl("file/workspace-1/attachment-1/manual.pdf")).toBe(true);
    expect(isInternalFileUrl("workspace-1/files/attachment-1/audio.mp3")).toBe(true);
    expect(isInternalFileUrl("https://old-docmost.example/api/files/attachment-1/manual.pdf")).toBe(true);
